...
class VehicleDetailAdminViewModel @Inject constructor(private val repository: IRepository) :
    ...
    fun acceptVehicle() {
        viewModelScope.launch {
            repository.acceptVehicle(_stateUi.value.vehicleDetail!!).collect { state ->
                when (state) {
                    is State.Loading -> {
                        _stateUi.update {
                            it.copy(isLoading = true)
                    ...
                    is State.Success -> {
                        _stateUi.update {
                            it.copy(
                                message = "Duyệt đơn đăng ký xe thành công",
                                isLoading = false,
                                isAccepted = true
                            )
                        }
    ...
    fun refuseVehicle() {
        viewModelScope.launch {
            repository.refuseVehicle(_stateUi.value.vehicleDetail!!).collect { state ->
                when (state) {
                    is State.Loading -> {
                        _stateUi.update {
                            it.copy(isLoading = true)
                    ...
                    is State.Success -> {
                        _stateUi.update {
                            it.copy(
                                message = "Từ chối duyệt đơn đăng ký xe thành công",
                                isLoading = false,
                                isRefuse = true
                            )
                        }
